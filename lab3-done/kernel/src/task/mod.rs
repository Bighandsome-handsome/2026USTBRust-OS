...
impl TaskManager {
    ...
    fn mark_current_suspended(&self) {
        ...
        inner.tasks[current].task_status = TaskStatus::Ready;           // add code
    }
    ...
    // None
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let status = inner.tasks[current].task_status as usize;

        // 计算当前任务所有系统调用的次数总和
        let syscall_times = inner.syscall_count[current].syscall_count.values().sum();
    ...
    fn get_total_syscall_count(&self) -> usize {
        ...
        let current = inner.current_task;
        // 直接求和
        inner.syscall_count[current].syscall_count.values().sum()
    }
}
