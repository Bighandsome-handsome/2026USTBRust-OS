...
pub fn sys_waitpid(pid: isize, exit_code_ptr: *mut i32) -> isize {
    ...
    // A. 判断有没有匹配的子进程
    let has_match = inner.children.iter().any(|child| {
        pid == -1 || child.getpid() == pid as usize
    });
    if !has_match {
        return -1;
    }

    // B. 判断匹配的子进程里有没有 Zombie
    let zombie_idx = inner.children.iter().enumerate().find(|(_, child)| {
        (pid == -1 || child.getpid() == pid as usize)
